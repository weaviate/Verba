      ...
      {links.map((link) => {
        ...
        return (
          <Link legacyBehavior key={link.name} href={link.path}>
            <a
              className={`mx-12 text-zinc-700 font-thin text-opacity-50 hover:scale-110 transform transition-all duration-500 hover:text-yellow-400 hover:text-opacity-100 ${
                isActive ? "text-white" : ""
              }`}
            >
              {link.name}

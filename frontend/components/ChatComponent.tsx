...
export function ChatComponent({ onUserMessageSubmit, isFetching }: ChatComponentProps) {
    ...
    const parseMessage = (content: string) => {
        ...
        content.replace(regex, (match, language, codeContent, index) => {
            if (index > lastEndIndex) {
                segments.push({ type: "text", content: content.slice(lastEndIndex, index) });
            }

            segments.push({ type: "code", language: language.trim(), content: codeContent.trim() });

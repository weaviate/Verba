          ...
          <span className="inline-block p-3 rounded-xl animate-press-in shadow-md font-mono text-sm bg-white">
            {parseMessage(previewMessage).map((segment, segIndex) => {
                if (segment.type === "text") {
                    ...
                    return (
                        <SyntaxHighlighter key={segIndex} language={segment.language} style={oneLight}
                                           className="rounded p-2">
                            {segment.content}
                        </SyntaxHighlighter>
                    );
            ...
            {showNotification && (
                <div
                    className="fixed bottom-5 left-5 animate-pop-in bg-green-500 text-white px-4 py-2 rounded text-sm shadow-md mt-4 mr-4 transition-opacity opacity-100">
                    Text copied!
                </div>
            )}
        </div>

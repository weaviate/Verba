            ...
            <div className="flex flex-col bg-bg-alt-verba rounded-lg shadow-lg p-5 text-text-verba gap-5 h-[65vh] overflow-auto w-full">
                ...
                <div className='grid grid-cols-1 lg:grid-cols-2 gap-2'>
                    {RAGComponents && Object.entries(RAGComponents.components).map(([key, value]) => (
                        <button key={"Component_" + key} disabled={!value.available} onClick={() => { onSelectComponent(key) }} className={`btn border-none ${value.available ? ("flex") : ("hidden")} ${key === RAGComponents.selected ? ("bg-secondary-verba text-text-verba") : ("bg-button-verba text-text-alt-verba")} hover:bg-button-hover-verba `}>
                            <p>
                                {key}
                            </p>
                        </button>

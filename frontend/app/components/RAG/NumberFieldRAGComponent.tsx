...
const NumberFieldRAGComponent: React.FC<NumberFieldRAGComponentProps> = ({ title, NumberFieldSetting, RAGConfig, RAGConfigTitle, RAGComponentTitle, setRAGConfig }) => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setRAGConfig((prevConfig: any) => {
            ...
            newConfig[RAGConfigTitle].components[RAGComponentTitle].config[title].value = newText
            return newConfig;
        });
    ...
    return (
        <div key={title} className='flex flex-col gap-1'>
            <div className='flex items-center justify-center lg:text-base text-sm'>
                <p>
                    {NumberFieldSetting.description}
                </p>
            </div>
            <div className='flex items-center justify-center'>
                <label className="input input-bordered flex items-center gap-2 w-full lg:text-base text-sm bg-bg-verba">
                    <input
                        ...
                        className="grow"
                        placeholder={title}
                        value={(RAGConfig[RAGConfigTitle].components[RAGComponentTitle].config as any)[title].value}

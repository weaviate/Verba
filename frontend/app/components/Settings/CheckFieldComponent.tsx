...
const CheckComponent: React.FC<CheckComponent> = ({ title, CheckboxSetting, setting, settingsConfig, setSettingsConfig }) => {
    ...
    return (
        <div key={title} className='flex flex-col gap-2'>
            <div className='flex items-center justify-center'>
                <p>
                    {CheckboxSetting.description}
                </p>
            </div>
            <div className='flex items-center justify-center'>
                <input type="checkbox" className="toggle" checked={(settingsConfig[setting].settings as any)[title].checked} onChange={handleChange} />
            </div>

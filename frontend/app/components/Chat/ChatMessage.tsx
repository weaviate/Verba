...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, handleCopyToBillboard, settingConfig }) => {

    return (
        <div className={`flex items-end gap-2 ${message.type === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`flex flex-col p-4 rounded-xl animate-press-in shadow-md sm:text-sm md:text-base ${message.type === "user" ? "bg-primary-verba" : "bg-bg-verba"}`}>
                {message.type === "system" ? (
                    <ReactMarkdown

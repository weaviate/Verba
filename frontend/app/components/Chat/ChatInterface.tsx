...
const ChatInterfaceComponent: React.FC<ChatInterfaceComponentProps> = ({
        ...
        localSocket.onmessage = (event) => {
            ...
            if (data.finish_reason === "stop") {
                ...
                if (data.cached) {
                    ...
                    setMessages(prev => [...prev, { type: "system", content: full_text, cached: true, distance: distance }]);
                } else {
                    setMessages(prev => [...prev, { type: "system", content: full_text }]);
                }
                setPreviewText("");
            }
        };
    ...
    const streamResponses = (query?: string, context?: string) => {
        if (socket?.readyState === WebSocket.OPEN) {
            const data = JSON.stringify({ query: query, context: context, conversation: messages });
            socket.send(data);
        } else {
    ...
    const getMessagesFromLocalStorage = (key: string) => {
        ...
        return [{ type: "system", content: settingConfig.Customization.settings.intro_message.text }]; // Return a default value or null if not found
    };
    ...
    const handleSendMessage = async (e?: React.FormEvent) => {
        ...
        if (sendInput.trim()) {
            ...
            if (textarea !== null) { // Check if the element is not null
    ...
    const fetchSuggestions = async (query: string) => {
        ...
        if (isFetchingSuggestion || query === "" || isFetching || !settingConfig.Chat.settings.suggestion.checked) {
            setSuggestions([])
            return
        }
    ...
    return (
        <div className='flex flex-col gap-2' >
            ...
            <div className="flex flex-col bg-bg-alt-verba rounded-lg shadow-lg p-2 md:p-5 text-text-verba gap-5 h-[55vh] overflow-auto">
                ...
                <div className='flex flex-col'>
                    {messages.map((message, index) => (
                        ...
                        >
                            <ChatMessage message={message} handleCopyToBillboard={handleCopyToBillboard} settingConfig={settingConfig} />
                        </div>
                    ))}
                    {/* Render the preview message if available */}
                    {previewText && (
                        <ChatMessage settingConfig={settingConfig} message={{ type: "system", content: previewText, cached: false }} handleCopyToBillboard={handleCopyToBillboard} />
                    )}
                </div>
            ...
            <div className="flex flex-col bg-bg-alt-verba rounded-lg shadow-lg p-5 text-text-verba gap-5 min-h-[9.3vh]">
                ...
                >
                    <textarea id={"reset"} rows={1} cols={10} onKeyDown={handleKeyDown} value={userInput} onChange={(e) => { setUserInput(e.target.value); fetchSuggestions(e.target.value) }} className=" bg-bg-alt-verba textarea textarea-xs p-2 text-sm md:text-base w-full" placeholder={`Ask ${settingConfig.Customization.settings.title.text} anything`}></textarea>

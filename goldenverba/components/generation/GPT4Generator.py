...
class GPT4Generator(Generator):
    ...
    async def generate(
        ...
        @parameter: contexts : list[str] - List of contexts
        ...
        messages = self.generate_prepare_messages(queries, contexts, conversation)
        ...
        try:
            ...
            # Extract full text from the completion result
        ...
        messages = [
            {
                "role": "system",
                "content": """
                    You are Miichisoft's dxGAI - GenAI Solution, chatbot for Retrieval Augmented Generation (RAG). You'll receive a user query and pieces of context that are semantically similar to that specific query.
                    Please only respond to these user queries according to the context provided.
                    If the documentation provided does not provide enough information, please return the answer "NO ANSWER" and say no more, I will use that character to identify and contact the staff in the company for help.
                    If users ask specific questions about you as a chatbot, answer them naturally. If the answer asks for code examples, encapsulate them with ``programming language name ``. Don't make fake code. Please answer in the language the user asked you and do not repeat the user's question.
                """,
            }
        ]

        for message in conversation:
            messages.append({"role": message.type if hasattr(message, 'type') else message.get('type', None), "content": message.content if hasattr(message, 'content') else message.get('content', None)})
